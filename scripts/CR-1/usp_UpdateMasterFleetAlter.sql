ALTER PROC usp_UpdateMasterFleet
AS 
BEGIN
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				SELECT	
					...
					NULL
				FROM MST_FLEET
				WHERE EMPLOYEE_ID = @EmployeeID AND VEHICLE_TYPE = 'BENEFIT';
			END
			FETCH NEXT FROM @XCursor 
			INTO @EmployeeID, @CostCenter, @CostCenterMstFleet;
		END
		...
		BEGIN
			...
			BEGIN
				...
				SELECT					
					...
					NULL
				FROM MST_FLEET
				WHERE EMPLOYEE_ID = @EmployeeID2 AND VEHICLE_TYPE = 'BENEFIT';
			END
			FETCH NEXT FROM @XCursor2 
			INTO @EmployeeID2, @BaseTown, @BaseTownMstFleet;
		END
		...
		BEGIN
			...
			BEGIN
				...
				SELECT
					...
					NULL
				FROM MST_FLEET
				WHERE EMPLOYEE_ID = @EmployeeID3 AND VEHICLE_TYPE = 'BENEFIT';
			END
			FETCH NEXT FROM @XCursor3 
			INTO @EmployeeID3, @FormalName, @FormalNameMstFleet;
		END
