...
AS BEGIN
	...
	USING (
		...
		C.VEHICLE_USAGE, C.GROUP_LEVEL, C.MODEL, C.COLOUR, C.POLICE_NUMBER, 
	...
	WHEN NOT MATCHED BY TARGET THEN
		...
		VEHICLE_USAGE, VEHICLE_GROUP_LEVEL, VEHICLE_MODEL, COLOR, POLICE_NUMBER, REMARK, REPORT_MONTH, REPORT_YEAR, CREATED_DATE,TRA_ID)
		...
		YEAR(Source.CREATED_DATE), Source.CREATED_DATE, Source.TRA_ID);
	...
	WHEN NOT MATCHED BY TARGET THEN
		...
		, Source.POLICE_NUMBER, Source.REMARK, MONTH(Source.CREATED_DATE), YEAR(Source.CREATED_DATE), Source.CREATED_DATE, Source.TRA_ID);
				
	MERGE KPI_REPORT_DATA AS Target
	USING (
		select 'CRF' AS FORM_TYPE, C.EMPLOYEE_ID, C.EMPLOYEE_NAME, C.EFFECTIVE_DATE, C.LOCATION_OFFICE,
		C.LOCATION_OFFICE_NEW, C.VEHICLE_USAGE, (SELECT EMP.GROUP_LEVEL FROM MST_EMPLOYEE EMP WHERE EMP.EMPLOYEE_ID = C.EMPLOYEE_ID) AS GROUP_LEVEL, C.MODEL, F.COLOR, F.POLICE_NUMBER, 
