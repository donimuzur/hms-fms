...
AS BEGIN
	...
	WHEN NOT MATCHED BY TARGET THEN
		...
		VEHICLE_USAGE, GROUP_LEVEL, MODEL, COLOR, POLICE_NUMBER, REMARK, REPORT_MONTH, REPORT_YEAR, CREATED_DATE)
	...
	MERGE KPI_REPORT_DATA AS Target
	USING (
		select 'CRF' AS FORM_TYPE, C.EMPLOYEE_ID, C.EMPLOYEE_NAME, C.EFFECTIVE_DATE, R.REASON, C.LOCATION_OFFICE,
		C.LOCATION_OFFICE_NEW, C.VEHICLE_USAGE, F.GROUP_LEVEL, C.MODEL, F.COLOR, F.POLICE_NUMBER, RE.REMARK, C.CREATED_DATE 
		FROM TRA_CRF C JOIN MST_REASON R ON C.REASON = R.MST_REASON_ID JOIN MST_FLEET F ON C.MST_FLEET_ID = F.MST_FLEET_ID
		JOIN MST_REMARK RE ON C.REMARK = RE.MST_REMARK_ID
	) AS Source
	On (Source.EMPLOYEE_ID = Target.EMPLOYEE_ID)
	AND (Source.CREATED_DATE = Target.CREATED_DATE)
	WHEN NOT MATCHED BY TARGET THEN
		INSERT ( FORM_TYPE, EMPLOYEE_ID, EMPLOYEE_NAME, EFFECTIVE_DATE, REASON, PREVIOUS_BASE_TOWN, NEW_BASE_TOWN, 
		VEHICLE_USAGE, GROUP_LEVEL, MODEL, COLOR, POLICE_NUMBER, REMARK, REPORT_MONTH, REPORT_YEAR, CREATED_DATE)
		VALUES( Source.FORM_TYPE, Source.EMPLOYEE_ID, Source.EMPLOYEE_NAME, Source.EFFECTIVE_DATE, Source.REASON
