...
AS BEGIN
	...
	AND (Source.MST_FlEET_ID = Target.MST_FlEET_ID)
	WHEN NOT MATCHED BY TARGET THEN
		...
		Source.GST, Source.TOTAL_MONTHLY_INSTALLMENT, Source.PO_NUMBER, Source.PO_LINE, MONTH(Source.CREATED_DATE), YEAR(Source.CREATED_DATE), getdate(), Source.EMPLOYEE_ID, Source.MST_FlEET_ID, Source.VEHICLE_FUNCTION)
	WHEN MATCHED THEN
	UPDATE SET POLICE_NUMBER = Source.POLICE_NUMBER,
				SUPPLY_METHOD = Source.SUPPLY_METHOD,
				...
				COLOR = Source.COLOR,
				CHASIS_NUMBER = Source.CHASIS_NUMBER,
				ENGINE_NUMBER = Source.ENGINE_NUMBER,
				VEHICLE_TYPE = Source.VEHICLE_TYPE,
				VEHICLE_USAGE = Source.VEHICLE_USAGE,
				START_CONTRACT = Source.START_CONTRACT,
				END_CONTRACT = Source.END_CONTRACT,
				VENDOR = Source.VENDOR_NAME,
				MONTHLY_INSTALLMENT = Source.MONTHLY_HMS_INSTALLMENT,
				GST = Source.GST,
				TOTAL_MONTHLY_INSTALLMENT = Source.TOTAL_MONTHLY_INSTALLMENT,
				PO_NUMBER = Source.PO_NUMBER,
				PO_LINE = Source.PO_LINE,
				REPORT_MONTH = MONTH(Source.CREATED_DATE),
				REPORT_YEAR = YEAR(Source.CREATED_DATE),
				EMPLOYEE_ID = Source.EMPLOYEE_ID,
				MST_FlEET_ID = Source.MST_FlEET_ID,
				VEHICLE_FUNCTION = Source.VEHICLE_FUNCTION;
END