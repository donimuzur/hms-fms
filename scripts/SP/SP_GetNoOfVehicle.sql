...
AS BEGIN
	...
	USING (
		...
		MONTH(CREATED_DATE) AS REPORT_MONTH, YEAR(CREATED_DATE) AS REPORT_YEAR, CREATED_DATE, REGIONAL FROM MST_FLEET
		WHERE (IS_ACTIVE = 1 )
		AND (SUPPLY_METHOD = 'Lease' OR SUPPLY_METHOD = 'Service' OR SUPPLY_METHOD = 'Temporary' OR SUPPLY_METHOD = 'Extend')
		GROUP BY VEHICLE_TYPE, SUPPLY_METHOD, VEHICLE_FUNCTION, CREATED_DATE, REGIONAL, MONTH(CREATED_DATE)
	) AS Source
	...
	WHEN NOT MATCHED BY TARGET THEN
		...
		YEAR(Source.CREATED_DATE), Source.CREATED_DATE, Source.REGIONAL);
