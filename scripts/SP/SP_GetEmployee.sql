...
MERGE MST_EMPLOYEE AS Target
	...
	UPDATE MST_EMPLOYEE SET IS_ACTIVE = 0
	WHERE not exists( SELECT * FROM VIEW_EMPLOYEE WHERE EMPLOYEE_ID = VIEW_EMPLOYEE.EMPLID )
and is_active = 1 and employee_id not like ('X%') AND NOT EXISTS(SELECT * FROM MST_FLEET FLT WHERE EMPLOYEE_ID = FLT.EMPLOYEE_ID AND FLT.IS_ACTIVE=1 )
