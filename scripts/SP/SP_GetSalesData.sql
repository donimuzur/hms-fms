...
AS BEGIN
	...
	USING (
		...
		JOIN MST_FLEET FL ON FO.POLICE_NUMBER = FL.POLICE_NUMBER
		RIGHT OUTER JOIN MST_SALES_VOLUME SV ON FL.REGIONAL = SV.REGION AND MONTH(FL.START_CONTRACT) = SV.[MONTH] AND YEAR(FL.START_CONTRACT) = SV.[YEAR] 
		WHERE (FL.VEHICLE_TYPE = 'WTC')
		AND (FL.VEHICLE_FUNCTION = 'SALES')
