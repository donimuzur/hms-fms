...
AS BEGIN
	...
	USING (
		select COUNT(C.TRA_CAF_ID) AS ACCIDENT_COUNT, F.VEHICLE_TYPE, F.REGIONAL, MONTH(C.INCIDENT_DATE) AS REPORT_MONTH, 
		YEAR(C.INCIDENT_DATE) AS REPORT_YEAR, C.INCIDENT_DATE
		FROM TRA_CAF C JOIN MST_FLEET F ON C.POLICE_NUMBER = F.POLICE_NUMBER
		GROUP BY F.VEHICLE_TYPE, F.REGIONAL, C.INCIDENT_DATE, MONTH(C.INCIDENT_DATE)
	) AS Source
	On (Source.REPORT_MONTH = Target.REPORT_MONTH)
	AND (Source.REPORT_YEAR = Target.REPORT_YEAR)
	AND (Source.INCIDENT_DATE = Target.CREATED_DATE)
	WHEN NOT MATCHED BY TARGET THEN
		...
		Source.REPORT_YEAR, Source.INCIDENT_DATE);
