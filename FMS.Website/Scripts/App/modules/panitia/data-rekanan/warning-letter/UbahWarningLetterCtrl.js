    ...
    function ctrl($http, $translate, $translatePartialLoader, $location, SocketService, WarningLetterService,
        ...
        function init() {
            if (vm.isAdd === true) {
                ...
                vm.action = "Ubah";
                vm.LetterID = item.item.LetterID;
                vm.warningLetter.RequestedDate = item.item.CreatedDate;
                vm.selectedVendorName = item.item.Vendor.VendorID;
                vm.selectedWarningType = item.item.SysReference.RefID;
                vm.warningLetter.StartDate = item.item.StartDate;
                vm.warningLetter.EndDate = item.item.EndDate;
                vm.Area = item.item.Area;
                vm.Description = item.item.Description;
                vm.selectedReporter = item.item.employee.ID;
            }
                   ...
                   if (response.status === 200) {
                       ...
                       if (vm.isAdd === false) {
                           for (var i = 0; i < vm.listReporter.length; i++) {
                               if (vm.selectedReporter === vm.listReporter[i].DepartmentID) {
                   ...
                   if (response.status === 200) {
                       ...
                       if (vm.isAdd === false) {
                           for (var i = 0; i < vm.listVendorName.length; i++) {
                               if (vm.selectedVendorName === vm.listVendorName[i].VendorID) {
                   ...
                   if (response.status === 200) {
                       ...
                       if (vm.isAdd === false) {
                           for (var i = 0; i < vm.listWarning.length; i++) {
                               if (vm.selectedWarning === vm.listWarning[i].RefID) {
