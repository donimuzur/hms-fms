(function () {
    ...
    ctrl.$inject = ['$http', '$translate', '$translatePartialLoader', '$location', 'SocketService',
        'EvaluationQualificationService', 'RoleService', 'EvaluationTechnicalService', 'UIControlService', '$uibModal', 'GlobalConstantService', '$stateParams', '$state'];
    function ctrl($http, $translate, $translatePartialLoader, $location, SocketService, EvaluationQualificationService,
        RoleService, EvaluationTechnicalService, UIControlService, $uibModal, GlobalConstantService, $stateParams, $state) {
        ...
        function kembali() {
            ...
            EvaluationTechnicalService.selectByEval(step, function (reply) {
                ...
                UIControlService.unloadLoading();
                if (reply.status === 200) {
                    vm.stepTender = reply.data;
                    console.info(JSON.stringify(reply.data));
                    $state.transitionTo('evaluation-technical', { TenderRefID: vm.TenderRefID, StepID: vm.stepTender[0].ID, ProcPackType: vm.ProcPackType });
                } else {
        ...
        function gotoDetail(data) {
            ...
            if (data.vendorOffer.DocumentType === "FORM_DOCUMENT") {
                $state.transitionTo('detail-commercial-qualf-teknis', { TenderRefID: data.evaluation.tender.tender.TenderRefID, ProcPackType: data.evaluation.tender.tender.ProcPackageType, VendorID: vm.VendorID });
            }
            else {
                var data = {
                    item: data
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: 'app/modules/panitia/evaluasi-teknis/detail-evaluation-qualification.html',
