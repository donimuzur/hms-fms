(function () {
    ...
    angular.module("app")
    .controller("KriteriaEvaluasiParentPrakualCtrl", ctrl);
    
    ctrl.$inject = ['$scope', '$http', '$uibModal', '$translate', '$translatePartialLoader', '$location', 'SocketService', 'KriteriaEvaluasiService', 'UIControlService'];
    /* @ngInject */
    function ctrl($scope, $http, $uibModal, $translate, $translatePartialLoader, $location, SocketService, KriteriaEvaluasiService, UIControlService) {
        var vm = this;
        var lang;
        ...
        function loadKriteria() {
            KriteriaEvaluasiService.countCriteria({
                ...
                if (reply.status === 200) {
                    ...
                    UIControlService.msg_growl("error", "Gagal mendapatkan jumlah kriteria evaluasi" );
                }
            }, function (err) {
            ...
            KriteriaEvaluasiService.selectCriteria({
                ...
                if (reply.status === 200) {
                    ...
                    if (data.length > 0) {
                        ...
                        UIControlService.msg_growl("error", "Data belum ada!");
                    }
                } else {
                    UIControlService.msg_growl("error", "Gagal mendapatkan data kriteria evaluasi" );
                }
            }, function (err) {
                UIControlService.msg_growl("error", "Gagal Akses API");
            });
        ...
        vm.col_defs = [
            {
                field: "obj",
                displayName: "  ",
                cellTemplate: '<a ng-show="row.branch.bisaNgatur && !(row.branch.IsMandatory && row.branch.ParentId != 0) && row.branch.level < 3" class="btn btn-flat btn-xs btn-primary" ng-click="cellTemplateScope.click(row.branch)"><i class="fa fa-plus-circle"></i>&nbsp; Tambah Sub Kriteria</a>',
                cellTemplateScope: {
            ...
            },
            {
                field: "obj",
                displayName: "  ",
                cellTemplate: '<a ng-show="row.branch.IsMandatory === false" ng-click="cellTemplateScope.click(row.branch)" title="edit" class="btn btn-flat btn-xs btn-success"><i class="fa fa-edit"></i>&nbsp; Ubah</a>',
                cellTemplateScope: {
                    click: function(data) {         // this works too: $scope.someMethod;
                        vm.ubah(data);
                    }
            ...
            },
            {
                field: "obj",
                displayName: "  ",
                cellTemplate: '<a ng-show="row.branch.IsMandatory === false" ng-click="cellTemplateScope.click(row.branch)" title="delete" class="btn btn-flat btn-xs btn-danger"><i class="fa fa-trash-o"></i>&nbsp; Hapus</a>',
                cellTemplateScope: {
                    click: function(data) {         // this works too: $scope.someMethod;
                        vm.hapus(data);
                    }
        ...
        function addMasterKriteria(level, id) {
            ...
            var modalInstance = $uibModal.open({
                templateUrl: 'app/modules/panitia/prakualifikasi/kriteria-evaluasi/formKriteriaEvaluasi.html',
        ...
        vm.ubah = ubah;
        function ubah(data) {
            var lempar = { criteriaId: data.CriteriaId, criteriaName: data.CriteriaName, level: data.Level };
            var modalInstance = $uibModal.open({
                templateUrl: 'app/modules/panitia/prakualifikasi/kriteria-evaluasi/formKriteriaEvaluasi.html',
                controller: 'formKriteriaEvaluasiCtrl',
                controllerAs: 'formKriteriaEvaluasiCtrl',
            ...
            bootbox.confirm(pesan, function (yes) {
                if (yes) {
                    ...
                    KriteriaEvaluasiService.deleteCriteria({
                        ...
                        if (reply.status === 200) {
                            UIControlService.msg_growl("notice", "Berhasil menghapus Kriteria/Sub-Kriteria");
                            vm.loadKriteria();
                        } else {
                            UIControlService.msg_growl("error", "Gagal menghapus Kriteria/Sub-Kriteria");
                        }
                    }, function (err) {
                        UIControlService.unloadLoading();
