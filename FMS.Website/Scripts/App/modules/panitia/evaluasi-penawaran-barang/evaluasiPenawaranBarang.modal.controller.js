(function () {
    ...
    angular.module("app")
    .controller("evaluasiPenawaranBarangModalController", ctrl);

    ctrl.$inject = ['$state', 'item', '$http', '$filter', '$uibModalInstance', '$translate', '$translatePartialLoader', '$location', 'SocketService', 'EvaluasiPenawaranBarangService', 'DataPengadaanService', 'UIControlService', 'UploaderService', 'UploadFileConfigService', 'GlobalConstantService'];
    /* @ngInject */
    function ctrl($state, item, $http, $filter, $uibModalInstance, $translate, $translatePartialLoader, $location, SocketService, EvaluasiPenawaranBarangService, DataPengadaanService, UIControlService, UploaderService, UploadFileConfigService, GlobalConstantService) {
        ...
        function saveEvaluation() {
            ...
            var evaluasi = {
                ID: evaluasiID,
                GoodsOfferEvaluationVendors: vm.evaluationVendor,
                TenderStepData: vm.tenderStepData
            }

            UIControlService.loadLoadingModal(loadmsg);
            EvaluasiPenawaranBarangService.saveEvaluation(evaluasi, function (reply) {
                UIControlService.unloadLoadingModal();
                if (reply.status == 200) {
