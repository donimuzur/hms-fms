(function () {
    ...
    angular.module("app").controller("MSTRSCtrl", ctrl);
  ...
  "10", "11", "12");
    ...
    .withPaginationType('full_numbers');
                    $scope.upload = function (file) {
                        ExcelReaderService.readExcel(file, function (reply) {
                            if (reply.status === 200) {
                                ...
                                console.info(sheet1);
                                if (sheet1[1].Column2 != "ReasonCategory") {
                                    UIControlService.msg_growl("error", "Column 2 Name Is " + sheet1[1].Column2);
                                    UIControlService.msg_growl("error", "Must be ReasonCategory");
                                    $uibModalInstance.dismiss('cancel');
                                }
                                if (sheet1[1].Column3 != "Reason") {
                                    UIControlService.msg_growl("error", "Column 3 Name Is " + sheet1[1].Column3);
                                    UIControlService.msg_growl("error", "Must be Reason");
                                    $uibModalInstance.dismiss('cancel');
                                }

                                for (var i = 2; i < sheet1.length; i++) {
                                    var item = {
                                        reason_category: sheet1[i].Column2,
                                        reason:sheet1[i].Column3 ,
                                        created_date: new Date(),
                                        created_by: localStorage.getItem('username'),
                                        last_modified: new Date(),
                                        modified_by: localStorage.getItem('username'),
        ...
        function toExcel(tabel) {
            ...
            alasql('SELECT reason_category AS `Reason Category`, reason1 AS `Reason`, datetime(last_modified) AS `Last Modified`,modified_by AS `Modified By` INTO XLSX("mst-reason.xlsx",{headers:true}) FROM ?', [vm.dataexcel]);
        }
