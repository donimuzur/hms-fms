(function () {
    ...
    angular.module("app").controller("RelocationWTCCtrl", ctrl);
 ...
 "Oct", "Nov", "Dec");
        ...
        function sendSubmitted(param) {
            RelocationService.SendCOPUser(data, function (reply) {
                if (reply.status === 200) {
                    UIControlService.msg_growl("notice", "Send CSF Draft Success");
                    vm.init();
                } else {
                ...
                controller: function ($uibModalInstance, $scope) {
                    ...
                    $scope.getEmployee = function (param) {
                        ...
                        RelocationService.GetEmployee({
                            Offset: $scope.offset,
                            Limit: $scope.pageSize}, function (reply) {
                            if (reply.status === 200) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance, $scope) {
                    ...
                    $scope.getLocation = function () {
                        $scope.location = {};
                        RelocationService.GetKota(function (reply) {
                            if (reply.status === 200) {
                                $scope.location = reply.data;
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance, $scope) {
                    $scope.getVehicle = function () {
                        $scope.DataVehicle = {};
                        RelocationService.GetVehicle(3, function (reply) {
                            if (reply.status === 200) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance, $scope) {
                    ...
                    $scope.getfleet = function () {
                        RelocationService.GetFleetCSF($stateParams.rq, function (reply) {
                            if (reply.status === 200) {
