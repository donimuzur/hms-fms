(function () {
    ...
    angular.module("app").controller("MSTRPCtrl", ctrl);
  ...
  "Oct", "Nov", "Dec");
        ...
        function loadRg() {
            ...
            if (vm.format(vm.modified) == "NaN-undefined-NaN" ||vm.format(vm.date) == "NaN-undefined-NaN") {
                var data = vm.regroup + "|" + vm.city + "|" + vm.address + "|" + vm.date + "|" + vm.modified + "|" + vm.by + "||" + offset + "|" + pageSize + "|" + newss;
            } else if (vm.format(vm.date) == "NaN-undefined-NaN") {
                var data = vm.regroup + "|" + vm.city + "|" + vm.address + "|" + vm.date + "|" + vm.format(vm.modified) + "|" + vm.by + "||" + offset + "|" + pageSize + "|" + newss;
            } else {
                var data = vm.regroup + "|" + vm.city + "|" + vm.address + "|" + vm.format(vm.date) + "|" + vm.format(vm.modified) + "|" + vm.by + "||" + offset + "|" + pageSize + "|" + newss;
            }
            UIControlService.unloadLoading();
            MstService.GetRgExcel(data, function (reply) {
                vm.dataexcel = reply.data.List;
            })
            MstService.GetRg(data,function (reply) {
                if (reply.status === 200) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance, $scope) {
                    ...
                    $scope.insert = function () {
                        $scope.loca_id;
                        console.log($scope.thn)
                        console.log($scope.tha)
...
.withPaginationType('full_numbers');
        ...
        function toExcel(tabel) {
            ...
            alasql('SELECT RegionalName AS `Regional Name`,city AS `City`, address AS `Address`, datetime(effective_date) AS `Effective Date`, datetime(last_modified) AS `Last Modified`,modified_by AS `Modified By` INTO XLSX("mst-regionalmapping.xlsx",{headers:true}) FROM ?', [vm.dataexcel]);
        }
