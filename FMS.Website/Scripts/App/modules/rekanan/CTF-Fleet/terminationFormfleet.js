(function () {
    ...
    function ctrl($http, $rootScope, $scope,
        ...
        function loadCTFEpaf(id) {
            console.log("ctf load");
            ctfservice.GetCTFEpafData(id,
                function succ(reply) {
                    if (reply.status === 200) {
                        vm.model = reply.data;
                        console.info("load ctf epaf", reply.data);
                    } else {
                        UIControlService.unloadLoading();
                        $.growl.error({ message: "Failed To Get Data Temporary" })
                        console.info("failed to load ctf epaf");
                    }
                });
        }
        ...
        function showExtendVehicle() {
            console.log("showExtend Vehicle");
                ...
                controller: function ($uibModalInstance, $scope) {
                    ...
                    $scope.getEmployee = function (param) {
                        ...
                        ctfservice.GetEmployee({
                            ...
                            function (reply) {
                                if (reply.status === 200) {
                                    $scope.EmployeeList = reply.data.List;
                    ...
                    $scope.selectEmployee = function (param) {
                        if (document.getElementById('csfID') != null) {
                            ...
                            vm.model.employeeId = param.employee_id;
                            vm.model.employeeName = param.employee_name;
