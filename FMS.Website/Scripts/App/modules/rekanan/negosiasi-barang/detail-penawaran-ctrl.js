(function () {
    ...
    angular.module("app").controller("DPVCtrl", ctrl);
    ...
    function ctrl($http, $translate, $translatePartialLoader, $location, SocketService, NegosiasiBarangService,
        ...
        function jLoad(current) {
            ...
            NegosiasiBarangService.itemallv(tender, function (reply) {
                ...
                if (reply.status === 200) {
                    ...
                    for (var i = 0; i < vm.nego.length; i++) {
                        if (i === 0) {
                            vm.cost = vm.nego[i].TotalPriceGOE;
                            vm.totalNego = vm.nego[i].TotalPrice;

                        }
                            else {
                            vm.cost = +vm.cost + +vm.nego[i].TotalPriceGOE;
        ...
        function editp() {
            ...
            NegosiasiBarangService.updatev(vm.data,
                                       function (reply) {
                                           UIControlService.loadLoading("Silahkan Tunggu...");
                                           if (reply.status === 200) {
        ...
        vm.back = back;
        function back() {
            $state.transitionTo('negosiasi-barang-vendor', { TenderRefID: vm.TenderRefID, StepID: vm.StepID, ProcPackType: vm.ProcPackType });
        }
        ...
        function save() {
            ...
            NegosiasiBarangService.updatedetailv(vm.list,
               function (reply) {
